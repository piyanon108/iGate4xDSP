...
QString SnmpStack::getsnmp_string_new(std::string ip, const char* oidname, int id, QString oidStringName, QString address)
{
    if (address.split(".").size() < 4)
    {
        QString arguments = QString("echo \"\" > /opt/iGatePlus/snmp/chID%1/%2_%3.txt").arg(id).arg(oidStringName).arg(address);
        system(arguments.toStdString().c_str());
        return "";
    }
    QString stringout = "";
    QString data = "";
    QString arguments = QString("snmpwalk -v 2c -t 0.1 -L f /tmp/log.text -c public %1 %2 > /opt/iGatePlus/snmp/chID%3/%4_%5.txt").arg(QString::fromStdString(ip)).arg(QString::fromStdString(oidname)).arg(id).arg(oidStringName).arg(address);
    system(arguments.toStdString().c_str());
    stringout = readLine(QString("/opt/iGatePlus/snmp/chID%1/%2_%3.txt").arg(id).arg(oidStringName).arg(address));
...
int SnmpStack::getsnmp_int_new(std::string ip,const char* oidname,int id, QString oidStringName, QString address)
{
    if (address.split(".").size() < 4)
    {
        QString arguments = QString("echo \"\" > /opt/iGatePlus/snmp/chID%1/%2_%3.txt").arg(id).arg(oidStringName).arg(address);
    ...
    QString arguments = QString("snmpwalk -v 2c -t 0.1 -L f /tmp/log.text -c public %1 %2 > /opt/iGatePlus/snmp/chID%3/%4_%5.txt").arg(QString::fromStdString(ip)).arg(QString::fromStdString(oidname)).arg(id).arg(oidStringName).arg(address);
    system(arguments.toStdString().c_str());
    lineOut = readLine(QString("/opt/iGatePlus/snmp/chID%1/%2_%3.txt").arg(id).arg(oidStringName).arg(address));
